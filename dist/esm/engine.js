...
function normalizeTransforms(transforms = []) {
  ...
  return transforms.map((transform, index) => {
    ...
    return {
      id: typeof transform.id === 'string' && transform.id.trim() ? transform.id.trim() : `transform_${index + 1}`,
...
export function createEngine(options = {}) {
  const transforms = normalizeTransforms(options.transforms);
  const codecSpecs = Array.isArray(options.codecs) && options.codecs.length > 0 ? options.codecs : AVAILABLE_CODECS;
  ...
  const alwaysPrefix = typeof options.alwaysPrefix === 'boolean' ? options.alwaysPrefix : codecEntries.length !== 1;
  const defaultCodec = typeof options.defaultCodec === 'undefined' ? codecEntries[0]?.id : normalizeCodecId(options.defaultCodec, 'default codec');
  ...
  async function compressDetailed(json) {
    ...
    if (candidates.length === 0) {
      if (skipped.length > 0) {
        throw createUnsupportedCodecError('engine', `None of the configured codecs are supported in this environment: ${skipped.map(entry => entry.codec).join(', ')}`);
      }
